...
def generate_text_numerical_features(df, text_col, target, verbose=True):
    ...
    df['total_chars_count']    = df[text_col].apply(len)
    
    #create total numer of words features in text column 
    df['num_words_count']      = df[text_col].apply(lambda x: len(x.split()))
    
    #create total numer of words features in text column 
    df['capital_chars_count']  = df[text_col].apply(lambda x: sum(1 for c in x if c.isupper()))
